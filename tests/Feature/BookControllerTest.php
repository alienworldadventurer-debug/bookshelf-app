...
class BookControllerTest extends TestCase
{
     ...
     *
     * @return void 1ページあたり10件の書籍が表示されることを確認する
     */
     ...
     *
     * @return void キーワードに一致する書籍だけが表示されることを確認する
     */
     ...
     *
     * @return void 指定ジャンルの書籍だけが表示されることを確認する
     */
     ...
     *
     * @return void 各ソート条件に応じた書籍順序を確認する
     */
    public function test_index_sorts_books_correctly(): void
    {
        ...
        $this->assertEquals($book1->id, $booksRating[0]->id);
        $this->assertEquals($book3->id, $booksRating[1]->id);
        $this->assertEquals($book2->id, $booksRating[2]->id);
    }

    /**
     * 検索・絞り込み・ソート条件がページネーションリンクに正しく引き継がれること
     *
     * @return void ページネーションリンクに検索条件が保持されることを確認する
     */
    public function test_index_pagination_retains_query_parameters(): void
    {
        ...
        Book::factory()->count(11)->create(['user_id' => $user->id])->each(function (Book $book) use ($genre): void {
    ...
    /**
     * ログイン・未ログインに応じて書籍詳細画面とアクションボタンが切り替わることを検証する。
     *
     * @return void 認証状態に応じた表示内容を確認する
     */
    public function test_show_displays_book_details_and_button_visibility(): void
    {
        ...
        $authResponse->assertSee('投稿する');
    }

    /**
     * 正しいISBNで検索した際、Google Books APIのモックを介して整形された書籍情報JSONが返却されること
     *
     * @return void モックAPIの情報が期待するJSONへ変換されることを確認する
     */
     ...
     *
     * @return void 書籍未検出時に404が返ることを確認する
     */
     ...
     *
     * @return void 不正なISBN形式に対して422が返ることを確認する
     */
     ...
     *
     * @return void 文字列以外のISBN入力が422で拒否されることを確認する
     */
     ...
     *
     * @return void 外部API障害時に500が返ることを確認する
     */
     ...
     *
     * @return void nullable項目が空でも書籍登録に成功することを確認する
     */
     ...
     *
     * @return void 所有者の書籍更新が成功することを確認する
     */
     ...
     *
     * @return void 書籍と関連データが削除されることを確認する
     */
     ...
     *
     * @return void 必須項目不足とISBN重複が拒否されることを確認する
     */
     ...
     *
     * @return void 他人の書籍への編集・更新が403で拒否されることを確認する
     */
     ...
     *
     * @return void 他書籍と重複するISBNの更新が拒否されることを確認する
     */
     ...
     *
     * @return void 他人の書籍の削除が拒否され、データが保持されることを確認する
     */
